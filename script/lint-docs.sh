...
# We are runing in a busybox docker container with very limited tooling, hence print
FILES=$(find "${GIT_ROOT}docs/" "${GIT_ROOT}script/ci_tool/" "${GIT_ROOT}devvm/" -type f -name "*.md" -print)
