...
func readStartedAtFile(sdriver driver.StorageDriver, targetPath string) (time.Time, error) {
	// todo:(richardscothern) - pass in a context
	startedAtBytes, err := sdriver.GetContent(context.Background(), targetPath)
	if err != nil {
		return time.Now(), err
	}
