...
| `name_exact`    | String | No       |         | Tag name filter. If set, tags are filtered using a exact match against its value and all sorting and pagination parameters are ignored. Only lowercase and uppercase letters, digits and underscores are allowed. Maximum of 128 characters. It must respect the `[a-zA-Z0-9_][a-zA-Z0-9._-]{0,127}` pattern. If the value is not valid, the `INVALID_QUERY_PARAMETER_VALUE` error is returned. Mutually exclusive with the `name` parameter. Returns a list with a single element if tag was found, empty list otherwise.                                  |
